...
const STAT: Record<string, { color: string; bg: string; border: string }> = {
  open:        { color: "#b08800", bg: "#fef9ec", border: "#f0dca0" },
  in_progress: { color: "#3b7dd8", bg: "#f0f5ff", border: "#b3d1ff" },
...
function BugsContent() {
  ...
  useEffect(() => {
    if (selectedId && bugs.length > 0) {
      const found = bugs.find((b) => b.id === selectedId);
  ...
  return (
    <div>
        ...
        {["", "open", "in_progress", "resolved", "closed"].map((s) => {
          ...
          return (
            <button
              ...
              style={{
                padding: "5px 14px", fontSize: "12px", fontFamily: t.font,
                cursor: "pointer", borderRadius: "5px",
                background: statusFilter === s ? (st?.bg || t.bgCard) : t.bgCard,
                border: `1px solid ${statusFilter === s ? (st?.border || t.borderDark) : t.border}`,
                color: statusFilter === s ? (st?.color || t.textMuted) : t.textMuted,
              }}
            >
              {s ? s.replace(/_/g, " ") : "all"}
            </button>
      ...
      {loading ? (
        ...
        <div style={{ color: t.error, fontSize: "13px", textAlign: "center", padding: "40px 0" }}>
          failed to load bug reports — check the API
        </div>
      ) : filtered.length === 0 ? (
        ...
        <div style={{ display: "flex", gap: "20px" }}>
          ...
          <div style={{ flex: selected ? "0 0 50%" : "1" }}>
            {filtered.map((bug) => {
              ...
              return (
                <div
                  key={bug.id}
                  onClick={() => select(bug.id)}
                  style={{
                    padding: "12px 16px",
                    background: selected?.id === bug.id ? t.bgAccent : t.bgCard,
                    border: `1px solid ${selected?.id === bug.id ? t.accentBorder : t.border}`,
          ...
          {selected && (
            <div style={{
              flex: "0 0 48%", background: t.bgCard, border: `1px solid ${t.border}`,
              borderRadius: "8px", padding: "20px", maxHeight: "80vh", overflow: "auto",
            }}>
              ...
              {selected.steps && (
                <div style={{ marginBottom: "16px" }}>
                  <div style={{ fontSize: "11px", color: t.textLight, fontWeight: 500, letterSpacing: "0.5px", marginBottom: "6px" }}>STEPS TO REPRODUCE</div>
                  <div style={{ padding: "10px", background: t.bgInput, border: `1px solid ${t.border}`, borderRadius: "6px", fontSize: "12px", color: t.textSecondary, whiteSpace: "pre-wrap" }}>{selected.steps}</div>
                </div>
              )}
              ...
              <div style={{ marginTop: "20px", borderTop: `1px solid ${t.border}`, paddingTop: "16px" }}>
                ...
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                  {["open", "in_progress", "resolved", "closed"].map((s) => {
                    const st = STAT[s] || STAT.open;
                    return s !== selected.reportStatus && (
                      <button
                        key={s}
                        onClick={() => updateStatus(selected.id, s)}
                        style={{ ...t.badge(st.color, st.bg, st.border), cursor: "pointer", padding: "4px 10px", fontFamily: t.font, fontSize: "11px" }}
                      >
                        {s.replace(/_/g, " ")}
                      </button>
                    );
