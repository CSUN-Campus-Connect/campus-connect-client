        ...
        <div style={{ display: "flex", gap: "20px" }}>
          <div style={{ flex: selected ? "0 0 50%" : "1" }}>
            {filtered.map((bug) => {
              ...
              return (
                <div key={bug.id} onClick={() => select(bug.id)} style={{ padding: "12px 16px", background: selected?.id === bug.id ? t.bgAccent : t.bgCard, border: `1px solid ${selected?.id === bug.id ? t.accentBorder : t.border}`, borderRadius: "6px", cursor: "pointer", marginBottom: "6px" }}>
...
export default function BugReportsPage() {
  return <Suspense fallback={<div style={{ color: "#bbb", fontSize: "13px" }}>loading...</div>}><BugsContent /></Suspense>;
}