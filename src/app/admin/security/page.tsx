...
const URGENCY_MAP: Record<string, { color: string; bg: string; border: string }> = {
  ...
  TIME_SENSITIVE: { color: "#7c3aed", bg: "#f5f0ff", border: "#d4bfff" },
};

const STATUS_MAP: Record<string, { color: string; bg: string; border: string }> = {
  SUBMITTED: { color: "#b08800", bg: "#fef9ec", border: "#f0dca0" },
  ACKNOWLEDGED: { color: "#3b7dd8", bg: "#f0f5ff", border: "#b3d1ff" },
  UNDER_REVIEW: { color: "#3b7dd8", bg: "#f0f5ff", border: "#b3d1ff" },
  INVESTIGATION: { color: "#7c3aed", bg: "#f5f0ff", border: "#d4bfff" },
  ESCALATED: { color: "#c94150", bg: "#fef2f3", border: "#f5c6cb" },
  PENDING_RESOLUTION: { color: "#b08800", bg: "#fef9ec", border: "#f0dca0" },
  RESOLVED: { color: "#2d8a4e", bg: "#f0faf4", border: "#c3e6cb" },
  CLOSED: { color: "#888", bg: "#f5f5f5", border: "#e0e0e0" },
  REOPENED: { color: "#c94150", bg: "#fef2f3", border: "#f5c6cb" },
};
...
function SecurityContent() {
  ...
  const openDetail = async (id: string) => {
    setDetailLoading(true); setTab("detail");
    try { const res = await api.get(`/api/v1/security/cases/${id}`, { headers }); setSelected(res.data); } catch {}
  ...
  const updateStatus = async (status: string) => {
    if (!selected) return;
    try { await api.patch(`/api/v1/security/cases/${selected.id}/status`, { status, note: statusNote }, { headers }); setStatusNote(""); openDetail(selected.id); fetchCases(statusFilter || undefined, deptFilter || undefined); } catch {}
  };

  const sendMessage = async () => {
    if (!selected || !messageText.trim()) return;
    try { await api.post(`/api/v1/security/cases/${selected.id}/messages`, { content: messageText, isInternal }, { headers }); setMessageText(""); openDetail(selected.id); } catch {}
  };

  const handleStatusFilter = (s: string) => { const next = statusFilter === s ? "" : s; setStatusFilter(next); fetchCases(next || undefined, deptFilter || undefined); };
  const handleDeptFilter = (d: string) => { const next = deptFilter === d ? "" : d; setDeptFilter(next); fetchCases(statusFilter || undefined, next || undefined); };
    ...
    <div>
      ...
      {stats && (
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
          {Object.entries(stats.byStatus).map(([key, count]) => {
            const sm = STATUS_MAP[key] || STATUS_MAP.SUBMITTED;
            return (
              <button key={key} onClick={() => handleStatusFilter(key)} style={{
                padding: "6px 14px", fontSize: "12px", fontFamily: t.font, cursor: "pointer", borderRadius: "5px",
                background: statusFilter === key ? sm.bg : t.bgCard, border: `1px solid ${statusFilter === key ? sm.border : t.border}`, color: sm.color,
              }}>
                <span style={{ fontWeight: 600, marginRight: "6px" }}>{count}</span>{key.replace(/_/g, " ").toLowerCase()}
              </button>
            );
          })}
        </div>
      ...
      <div style={{ display: "flex", gap: "6px", marginBottom: "20px", flexWrap: "wrap" }}>
        {Object.entries(DEPT_LABELS).map(([key, label]) => (
          <button key={key} onClick={() => handleDeptFilter(key)} style={{
            ...
            background: deptFilter === key ? t.bgAccent : t.bgCard, border: `1px solid ${deptFilter === key ? t.accentBorder : t.border}`,
      ...
      <div style={{ display: "flex", gap: "20px" }}>
        <div style={{ flex: selected ? "0 0 45%" : "1" }}>
          {loading ? <div style={{ color: t.textLight, fontSize: "13px" }}>loading...</div> : cases.length === 0 ? (
            <div style={{ color: t.textLight, fontSize: "13px", textAlign: "center", padding: "40px 0" }}>no cases found</div>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
              {cases.map((c) => {
                const um = URGENCY_MAP[c.urgency] || URGENCY_MAP.MEDIUM;
                const sm = STATUS_MAP[c.status] || STATUS_MAP.SUBMITTED;
                return (
                  <div key={c.id} onClick={() => select(c.id)} style={{
                    padding: "12px 16px", background: selected?.id === c.id ? t.bgAccent : t.bgCard,
                    border: `1px solid ${selected?.id === c.id ? t.accentBorder : t.border}`, borderRadius: "6px", cursor: "pointer",
                  }}>
                    <div style={{ display: "flex", gap: "6px", alignItems: "center", marginBottom: "4px", flexWrap: "wrap" }}>
                      <span style={{ fontSize: "11px", color: t.textLight }}>{c.caseNumber}</span>
                      <span style={t.badge(um.color, um.bg, um.border)}>{c.urgency}</span>
                      <span style={t.badge(sm.color, sm.bg, sm.border)}>{c.status.replace(/_/g, " ")}</span>
                      <span style={t.badge(t.textMuted, t.bgCard, t.borderDark)}>{DEPT_LABELS[c.assignedDepartment] || c.assignedDepartment}</span>
                    </div>
                    <div style={{ fontSize: "13px", color: t.textPrimary, fontWeight: 500 }}>{c.title}</div>
                    <div style={{ fontSize: "11px", color: t.textLight, marginTop: "4px" }}>
                      ...
                      {" · "}{c.reportType.replace(/_/g, " ").toLowerCase()}{" · "}{new Date(c.createdAt).toLocaleDateString()}
                    </div>
                  </div>
                );
              })}
        ...
        {selected && (
          <div style={{ flex: "0 0 53%", background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: "8px", maxHeight: "85vh", overflow: "auto" }}>
            {detailLoading ? <div style={{ padding: "20px", color: t.textLight, fontSize: "13px" }}>loading...</div> : (
              <>
                <div style={{ padding: "16px 20px", borderBottom: `1px solid ${t.border}`, display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontSize: "12px", color: t.textLight }}>{selected.caseNumber}</div>
                    <div style={{ fontSize: "16px", fontWeight: 600, color: t.textPrimary, marginTop: "4px" }}>{selected.title}</div>
                  </div>
                ...
                <div style={{ display: "flex", borderBottom: `1px solid ${t.border}` }}>
                  {(["detail", "evidence", "messages", "history", "parties"] as const).map((tb) => (
                    <button key={tb} onClick={() => setTab(tb)} style={{
                      padding: "8px 16px", fontSize: "12px", fontFamily: t.font, cursor: "pointer",
                      background: "transparent", border: "none", borderBottom: tab === tb ? `2px solid ${t.accent}` : "2px solid transparent",
                ...
                <div style={{ padding: "16px 20px" }}>
                  {tab === "detail" && (
                    <>
                      <div style={{ display: "flex", gap: "6px", marginBottom: "16px", flexWrap: "wrap" }}>
                        ...
                        <span style={t.badge((STATUS_MAP[selected.status] || STATUS_MAP.SUBMITTED).color, (STATUS_MAP[selected.status] || STATUS_MAP.SUBMITTED).bg, (STATUS_MAP[selected.status] || STATUS_MAP.SUBMITTED).border)}>{selected.status.replace(/_/g, " ")}</span>
                      ...
                      <div style={{ marginTop: "16px", padding: "12px", background: t.bgInput, border: `1px solid ${t.border}`, borderRadius: "6px", fontSize: "13px", color: t.textSecondary, lineHeight: "1.6", whiteSpace: "pre-wrap" }}>{selected.description}</div>
                        ...
                        <div style={{ marginTop: "20px", borderTop: `1px solid ${t.border}`, paddingTop: "16px" }}>
                          ...
                          <div style={{ display: "flex", gap: "6px", flexWrap: "wrap" }}>
                            {["ACKNOWLEDGED", "UNDER_REVIEW", "INVESTIGATION", "ESCALATED", "PENDING_RESOLUTION", "RESOLVED", "CLOSED", "REOPENED"].map((s) => {
                              ...
                              return <button key={s} onClick={() => updateStatus(s)} style={{ ...t.badge(sm.color, sm.bg, sm.border), cursor: "pointer", padding: "4px 10px", fontFamily: t.font, fontSize: "11px" }}>{s.replace(/_/g, " ").toLowerCase()}</button>;
                            })}
                          </div>
                        </div>
                      )}
                  ...
                  {tab === "evidence" && (
                    <>
                      {selected.evidence.length === 0 ? <div style={{ color: t.textLight, fontSize: "13px", textAlign: "center", padding: "20px 0" }}>no evidence uploaded</div> : (
                              ...
                              <div key={e.id} style={{ border: `1px solid ${t.border}`, borderRadius: "6px", overflow: "hidden" }}>
                                {isImage && (
                                  <div onClick={() => setEnlargedEvidence(isEnlarged ? null : e.id)} style={{ cursor: "pointer", background: t.bgInput, display: "flex", justifyContent: "center", padding: "8px" }}>
                                    <img src={e.fileUrl} alt={e.fileName} style={{ maxWidth: isEnlarged ? "100%" : "200px", maxHeight: isEnlarged ? "none" : "140px", objectFit: isEnlarged ? "contain" : "cover", transition: "all 0.2s" }} />
                                  </div>
                                )}
                                {!isImage && (
                                ...
                                <div style={{ padding: "10px 12px" }}>
                                  <div style={{ display: "flex", justifyContent: "space-between" }}><div style={{ fontSize: "12px", color: t.textPrimary, fontWeight: 500 }}>{e.fileName}</div><div style={{ fontSize: "10px", color: t.textLight }}>{(e.fileSizeBytes / 1024).toFixed(1)} KB</div></div>
                                  {e.description && <div style={{ fontSize: "12px", color: t.textMuted, marginTop: "4px" }}>{e.description}</div>}
                                  <div style={{ fontSize: "10px", color: t.textLight, marginTop: "6px" }}>sha256: {e.checksumSha256}</div>
                                  <div style={{ fontSize: "10px", color: t.textLight, marginTop: "2px" }}>{new Date(e.uploadedAt).toLocaleString()}{e.isRedacted && <span style={{ color: t.error, marginLeft: "8px", fontWeight: 500 }}>REDACTED</span>}</div>
                                </div>
                              </div>
                            );
                          })}
                  ...
                  {tab === "messages" && (
                    <>
                      {selected.messages.length === 0 ? <div style={{ color: t.textLight, fontSize: "13px", textAlign: "center", padding: "20px 0" }}>no messages</div> : (
                        <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "16px" }}>
                          {selected.messages.map((m: any) => (
                            <div key={m.id} style={{ padding: "10px 12px", borderRadius: "6px", background: m.isInternal ? t.warningBg : t.bgInput, border: `1px solid ${m.isInternal ? t.warningBorder : t.border}`, borderLeft: `3px solid ${m.senderRole === "REPORTER" ? t.info : m.isInternal ? t.warning : t.success}` }}>
                              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "4px" }}>
                                <span style={{ fontSize: "11px", color: m.senderRole === "REPORTER" ? t.info : t.success, fontWeight: 500 }}>{m.senderRole.toLowerCase()}{m.isInternal ? " (internal)" : ""}</span>
                      ...
                      {hasPermission(permissions, "security:investigate") && (
                        <div>
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <label style={{ fontSize: "11px", color: t.textMuted, display: "flex", alignItems: "center", gap: "6px", cursor: "pointer" }}><input type="checkbox" checked={isInternal} onChange={(e) => setIsInternal(e.target.checked)} />internal only</label>
                  ...
                  {tab === "history" && (
                    <>
                      {selected.statusHistory.length === 0 ? <div style={{ color: t.textLight, fontSize: "13px", textAlign: "center", padding: "20px 0" }}>no history</div> : (
                        <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                          {selected.statusHistory.map((h: any) => {
                            ...
                            return (
                              <div key={h.id} style={{ padding: "8px 12px", borderLeft: `2px solid ${sm.border}`, fontSize: "12px" }}>
                                <span style={{ color: t.textMuted }}>{h.previousStatus}</span><span style={{ color: t.textLight }}> → </span><span style={{ color: sm.color, fontWeight: 500 }}>{h.newStatus}</span>
                  ...
                  {tab === "parties" && (
                    <>
                      {selected.involvedParties.length === 0 ? <div style={{ color: t.textLight, fontSize: "13px", textAlign: "center", padding: "20px 0" }}>no involved parties recorded</div> : (
...
export default function SecurityAdminPage() {
  return <Suspense fallback={<div style={{ color: "#bbb", fontSize: "13px" }}>loading...</div>}><SecurityContent /></Suspense>;
}