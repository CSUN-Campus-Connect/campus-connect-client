...
import {
  Box, CssBaseline, Card, Button, Dialog, DialogTitle, DialogContent,
  DialogActions, Fab, Stack, Divider, IconButton,
} from "@mui/material";
...
export default function DashboardPage() {
  ...
  const removeWidgetById = (id: string) => {
    const grid = gridInstance.current;
    if (!grid) return;
    const el = grid.engine.nodes.find(n => n.el?.getAttribute("gs-id") === id)?.el;
    if (el) grid.removeWidget(el);
    saveLayout();
  };

  const addWidget = async (id: string, x: number, y: number, w: number, h: number, content: React.ReactNode) => {
    const grid = gridInstance.current;
    if (!grid) return;
  ...
  const addSticky = () => {
    ...
    addWidget(id, 0, 0, 3, 2, <StickyNoteWidget id={id} onDelete={() => removeWidgetById(id)} />);
  };
  const addCampusEvents = () => {
    const id = `events-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 4, 3, <CampusEventTrackerWidget onDelete={() => removeWidgetById(id)} />);
  };
  const addDM = () => {
    const id = `dm-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 3, 2, <DMNotificationsWidget onDelete={() => removeWidgetById(id)} />);
  };
  const addWeather = () => {
    const id = `weather-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 3, 2, <WeatherWidget onDelete={() => removeWidgetById(id)} />);
  };
  const addCalendar = () => {
    const id = `cal-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 4, 4,
  ...
  const addAssignmentTracker = () => {
    const id = `canvas-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 5, 3, <AssignmentTrackerWidget onDelete={() => removeWidgetById(id)} />);
  };
  const addCitationGenerator = () => {
    const id = `citer-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 5, 4, <CitationGeneratorWidget onDelete={() => removeWidgetById(id)} />);
  };
  const addMarketplaceSuggest = () => {
    const id = `mkt-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 4, 3, <MarketplaceSuggestWidget onDelete={() => removeWidgetById(id)} />);
  };
  const addPhotoPin = () => {
    const id = `photo-${Math.random().toString(36).slice(2, 7)}`;
    addWidget(id, 0, 0, 4, 4, <PhotoPinWidget id={id} onDelete={() => removeWidgetById(id)} />);
  };
  ...
  const addWorkoutDeck = () => {
    ...
    addWidget(id, 0, 0, 4, 4, <WorkoutDeckWidget onDelete={() => removeWidgetById(id)} />);
  };
  ...
  React.useEffect(() => {
    ...
    const grid = GridStack.init(
      { float: true, cellHeight: 110, minRow: 1, draggable: { handle: ".widget-drag" }, resizable: { handles: "se, s, e" }, margin: 8 },
      gridRef.current
    );

    addWidget("kpi1", 0, 0, 3, 1, <KPI label="Current GPA" value="3.67" hint="+0.12 from last semester" onDelete={() => removeWidgetById("kpi1")} />);
    addWidget("kpi2", 3, 0, 3, 1, <KPI label="Assignments Done" value={String(assignmentsDone)} hint="this term" onDelete={() => removeWidgetById("kpi2")} />);
    addWidget("kpi3", 6, 0, 3, 1, <KPI label="Pending Assignments" value={String(classes.reduce((s, c) => s + c.pending, 0))} hint="across classes" onDelete={() => removeWidgetById("kpi3")} />);
    addWidget("kpi4", 9, 0, 3, 1, <KPI label="Clubs Joined" value="4" onDelete={() => removeWidgetById("kpi4")} />);

    addWidget("calendar", 0, 1, 4, 4,
      <Card sx={{ height: "100%", display: "flex", flexDirection: "column" }}>
        <Box sx={{ p: 1 }}><CalendarCard onClose={() => removeWidgetById("calendar")} /></Box>
      </Card>
    );
    ...
    grid.on("dragstop", (_event, el: HTMLElement) => {
      ...
      if (!dragged) { saveLayout(); return; }
      const r = el.getBoundingClientRect();
      const cx = r.left + r.width / 2;
      const cy = r.top + r.height / 2;
      const targetEl = document.elementFromPoint(cx, cy)?.closest(".grid-stack-item") as HTMLElement | null;
      if (!targetEl || targetEl === el) { saveLayout(); return; }
      const target: any = (targetEl as any).gridstackNode;
      if (!target) { saveLayout(); return; }
      const tr = targetEl.getBoundingClientRect();
      const leftHalf = cx < tr.left + tr.width / 2;
      const cols = grid.getColumn();
      const y = target.y;
      const newDraggedX = leftHalf ? target.x : Math.min(cols - dragged.w, target.x + Math.max(0, target.w - dragged.w));
      const newTargetX = leftHalf ? Math.min(cols - target.w, target.x + dragged.w) : Math.max(0, target.x - dragged.w);
      const origDragged = { x: dragged.x, y: dragged.y, w: dragged.w, h: dragged.h };
      const origTarget = { x: target.x, y: target.y, w: target.w, h: target.h };
      grid.batchUpdate();
      try {
        ...
        const landed = dNode && tNode && dNode.x === newDraggedX && dNode.y === y && tNode.x === newTargetX && tNode.y === y;
        if (!landed) { grid.update(targetEl, origTarget); grid.update(el, origDragged); }
  ...
  return (
    <Box sx={{ display: "flex", bgcolor: "#fafafb", minHeight: "100vh" }}>
      <CssBaseline />
      <DashboardSidebar drawerWidth={drawerWidth} onLogout={handleLogout} onWidthChange={setDrawerWidth} />
      <Box component="main" sx={{
        position: "relative", flexGrow: 1, p: 3, ml: `${drawerWidth}px`,
        width: `calc(100% - ${drawerWidth}px)`,
        transition: "margin-left 0.28s cubic-bezier(0.4, 0, 0.2, 1), width 0.28s cubic-bezier(0.4, 0, 0.2, 1)", minWidth: 0,
      }}>
        <NewAccountSetup />
        <div ref={gridRef} className="grid-stack" />
        <Fab onClick={openLibrary} sx={{ position: "fixed", right: 24, bottom: 24 }} color="primary">
          <AddIcon />
        </Fab>
      </Box>
      ...
      <Dialog open={addOpen} onClose={closeLibrary} fullWidth maxWidth="sm">
        <DialogTitle sx={{ pr: 6 }}>
          Add a widget
          <IconButton onClick={closeLibrary} size="small" sx={{ position: "absolute", top: 8, right: 8 }} aria-label="close">
            <CloseIcon />
          </IconButton>
        </DialogTitle>
        <DialogContent>
          <Stack spacing={1.5} mt={1}>
            <Button variant="contained" color="success" onClick={addMatadorCompass} sx={{ fontWeight: 700, py: 1.5 }}>
              Matador Compass (Student Success Guide)
            </Button>

            <Divider />

            <Button variant="outlined" onClick={addSticky}>Sticky Note</Button>
            <Button variant="outlined" onClick={addCampusEvents}>Campus Events Tracker</Button>
            <Button variant="outlined" onClick={addDM}>DM Notifications</Button>
            <Button variant="outlined" onClick={addCalendar}>Calendar</Button>
            <Button variant="outlined" onClick={addWeather}>CSUN Weather</Button>
            <Button variant="outlined" onClick={() => {
              const id = `today-${Math.random().toString(36).slice(2, 7)}`;
              addWidget(id, 0, 0, 3, 4, <ClassHelperWidget onDelete={() => removeWidgetById(id)} />);
            }}>Class Helper</Button>

            <Divider />

            <Button variant="contained" onClick={addAssignmentTracker}>Assignment Tracker</Button>
            <Button variant="contained" onClick={addCitationGenerator}>Citation Generator (APA/MLA)</Button>
