    ...
    >
      <div
        style={{
          position: 'absolute',
          inset: 0,
          background: 'linear-gradient(160deg, #A80532 0%, #7a0222 40%, #5a0118 100%)',
        }}
      />
      <div
        style={{
          ...
          filter: 'blur(40px)',
          animation: 'mp-orb1 20s ease-in-out infinite',
        }}
      />
      <div
        style={{
          ...
          height: '55vw',
          borderRadius: '50%',
          background: 'radial-gradient(circle, rgba(255,50,50,0.12) 0%, transparent 60%)',
          filter: 'blur(50px)',
          animation: 'mp-orb2 25s ease-in-out infinite',
        }}
      />
      <div
        style={{
          ...
          filter: 'blur(60px)',
          animation: 'mp-orb3 30s ease-in-out infinite',
        }}
      />
      <div
        style={{
...
function HeroSection({ favCount, favBtnRef, onFavClick, onBack, onSell }: HeroProps) {
  return (
    <div
      style={{
        position: 'relative',
    ...
    >
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0.875rem 2rem' }}>
        <div style={{ display: 'flex', gap: 10, marginBottom: 12 }}>
          ...
          >
            {SORT_OPTIONS.map((o) => (
              <option key={o.value} value={o.value} style={{ background: '#7a0222' }}>
                {o.label}
              </option>
            ))}
          </select>
          ...
          {['all', ...LISTING_TYPES.map((t) => t.value)].map((val) => {
            ...
            return (
              <button
                ...
                onMouseEnter={(e) => {
                  if (!active) e.currentTarget.style.background = 'rgba(255,255,255,0.18)';
                }}
                onMouseLeave={(e) => {
        ...
        >
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <main style={{ maxWidth: 1280, margin: '0 auto', padding: '2rem 2rem 4rem' }}>
              ...
              {!mp.loading && mp.error && (
                <ErrorState error={mp.error} onRetry={mp.triggerRefresh} />
              )}
                  ...
                  {mp.listings.map((item) => (
                    <li key={item.id}>
                      <MarketplaceCard
                        ...
                        onDelete={async (id) => {
                          const ok = await mp.deleteListing(id);
                          if (!ok) alert('Failed to delete listing.');
                        }}
